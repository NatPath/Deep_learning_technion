...
class LeakyReLU(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        alpha_tensor = torch.mul(x, self.alpha)
        out = torch.max(x, alpha_tensor)
        # ========================
...
class ReLU(LeakyReLU):
    ...
    def __init__(self):
        # ====== YOUR CODE: ======
        LeakyReLU.__init__(self, alpha = 0)
        # ========================
...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        # Implement gradient w.r.t. the input x
        # ====== YOUR CODE: ======
        sig = self.grad_cache["sig"]
        dx = torch.mul(sig, 1-sig)
        dx = torch.mul(dx, dout)
        # ========================
...
class Linear(Layer):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = torch.normal(torch.zeros(out_features, in_features), torch.full((out_features, in_features), wstd))
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        exp_sum_log = torch.log(torch.sum(torch.exp(x), 1))
        x_y = x[torch.arange(x.size(0)), y]
        loss = torch.mean(exp_sum_log - x_y)
        # ========================
