...
class FlipUpDown(object):
    def __call__(self, x: torch.Tensor):
        ...
        '''
        C,H,W=x.size()
        new_tensor=[[[x[c][H-1-h][w].item() for w in range(W)] for h in range(H)] for c in range(C)]
...
class BiasTrick(object):
    ...
    def __call__(self, x: torch.Tensor):
        ...
        shape_of_vec = tuple(torch.tensor(x.shape[:-1]).tolist()+[1])
        ones_vec = torch.ones(shape_of_vec,dtype= x.dtype)
        tensors=(ones_vec,x)
        return torch.cat(tensors,x.dim()-1)
